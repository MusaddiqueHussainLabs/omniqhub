...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        try
        {
            var response = await httpClient.GetAsync("http://localhost:8080/api/v1/documents", cancellationToken);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                documents = await JsonSerializer.DeserializeAsync<List<DocumentResponse>>(stream, options, cancellationToken);
            }
        }
